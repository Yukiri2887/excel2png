...
import com.example.excel.entity.Grid;
import com.example.excel.entity.UserCell;
...
//     * @param excelUrl excel路径
     ...
     */
    public static List<String> excelToImage(File convertFile, String path) throws IOException, InvalidFormatException {
        Workbook wb = WorkbookFactory.create(convertFile);
        wb.close();
        // 获取每个Sheet表
...
//            }
            ...
            // 绘图
            pngUrl= draw(imageWidth, imageHeight, grids, path);
            downList.add(pngUrl);
        }
        return downList;
    }
        ...
        for (int i = 0; i < rowSize; i++) {
            for (int j = 0; j < colSize; j++) {
                ...
                if (cell.getCellTypeEnum() == CellType.NUMERIC) {
                    ...
                    strCell = NumberToTextConverter.toText(cell.getNumericCellValue());//重点,无精度缺失的确切值.;
                }
                else {
                    strCell = "";
                }
//                 如果为空可能单元格是图片
//                if ("".equals(strCell)&&dataMap!=null) {
...
//                }
    ...
    public static String draw(int imageWidth, int imageHeight, List<Grid> grids, String path) throws IOException {
        ...
        String timeStamp=dateFormat.format(calendar.getTime()).toString();
        // 若该文件夹不存在，则创建一个文件夹
        File filePath = new File( path +timeStamp);
        if (!filePath.exists()) {
            filePath.mkdirs();
        }
        String currentTime= String.valueOf(System.currentTimeMillis());
        String imgUrl = path +timeStamp+ File.separator + currentTime + ".png";
        ...
        return  p.getProperty("url")+timeStamp+ File.separator + currentTime + ".png";
    }
